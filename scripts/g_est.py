...
# VN layers
from utils.model import GravityEstimationModel
...
def train(VN_model, classical_model, train_loader, test_loader, criterion, optimizer, optimizer_classical, device):
    ...
    for epoch in range(EPOCHS):
        ...
        if epoch == 0:
            ...
            torch.save(VN_model.state_dict(), "checkpoints/best_vn_model.pth")
            torch.save(classical_model.state_dict(), "checkpoints/best_classical_model.pth")
        else:
...
if __name__ == "__main__":
    ...
    if os.path.exists("checkpoints/best_vn_model.pth"):
        VN_model.load_state_dict(torch.load("checkpoints/best_vn_model.pth"))
        classical_model.load_state_dict(torch.load("checkpoints/best_classical_model.pth"))
